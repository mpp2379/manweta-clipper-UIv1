...
export const Step2Transcribe: React.FC<Step2TranscribeProps> = ({
  ...
  const [currentStatusText, setCurrentStatusText] = useState('Listening to your video audio...');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (wordIndex < words.length) {
        ...
        if (newProg < 40) {
          setCurrentStatusText('Transcribing spoken speech and conversations...');
        } else if (newProg < 75) {
          setCurrentStatusText('Timing each word for smooth animated subtitles...');
        } else {
          setCurrentStatusText('Finishing up your editable transcript...');
        }
        wordIndex++;
      } else {
        clearInterval(interval);
        setProgress(100);
        setCurrentStatusText('Transcription complete! Ready to find highlights.');
        setIsCompleted(true);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium bg-[#141414] text-[#00FF85] border border-[#222222] mb-2">
            <span>Step 2: Generate Transcript</span>
            <span className="text-[#444444]">•</span>
            <span className="text-[11px] text-[#888888]">Automatic Speech Recognition</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-light text-white tracking-tight">
            Transcribing Video to Word-by-Word Text
          </h2>
          <p className="text-xs sm:text-sm text-[#888888] mt-1 max-w-2xl">
            Sit back while our AI listens to your audio and creates word-by-word timestamps for animated captions.
          </p>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full text-[10px] bg-[#181818] text-[#00FF85] border border-[#2A2A2A]">
                  High Accuracy
                </span>
              </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-semibold uppercase tracking-widest text-[#888888] flex items-center gap-1.5">
            ...
            Live Word-by-Word Preview ({streamedWords.length} words detected)
          </h4>
          <span className="text-[11px] text-[#666666]">
            Captions synchronized for your video
          </span>
        </div>
